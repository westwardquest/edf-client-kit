...
| Goal | Steps |
| ---- | ----- |
| **Fetch ticket data** | MCP **`list_priority_active_tickets`** (optional `band`), **`get_ticket`**, **`get_ticket_by_number`**, **`search_tickets`** — each success refreshes **`.warpdesk/.ticket_selector`**. Or **`npm run warpdesk:tickets`** / CLI **`list`**. Open **`.warpdesk/.ticket_selector`** in WarpDesk Tools. |
