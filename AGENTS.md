...
2. After pushing the branch or opening a PR, set the ticket’s **`code_link_url`** in the app, or put it in a **draft** (`draft_ticket_update` / YAML) and have a human **apply** it in **edf-tools** (or the app / raw HTTP API), or use **`npm run edf:ticket:patch`** / **`PATCH`** from automation—so work is traceable.
...
**Prefer the `edf-tickets` MCP tools** whenever they appear in your tool list (`list_tickets`, `list_priority_active_tickets`, `get_ticket`, `draft_ticket_update`, `search_tickets`, `bootstrap_workspace`; plus **`update_ticket`** / **`add_ticket_comment`** only if **`EDF_MCP_ALLOW_DIRECT_UPDATES=1`**). They call the same HTTP API as the app. For **mutations**, use **`draft_ticket_update`** only to write the draft file; tell the user to **Confirm** in **edf-tools** (do not apply via tools or terminal).
...
Or run **`node vendor/edf-client-kit/mcp/tickets-cli.mjs`** with subcommands `list` (**`--queue`** for the active priority queue), `get`, `lookup`, **`patch`**, **`draft`**.
...
- Apply ticket updates or post comments **via MCP** without going through **`draft_ticket_update`** → human review → **Confirm in edf-tools** (use the app UI or CLI **`patch`** if you are not using MCP; there is no MCP or CLI apply-draft for agents).
